...
class PanelFormService
{
    public function __construct(protected PanelContract &$panelContract)
    {
    }
    ...
    public function formCreate(array $params = []): string
    {
        $fields = $this->getFields('create');
        $row = $this->panelContract->getRow();
        $res = '';
    ...
    public function formEdit(array $params = []): string
    {
        ...
        $res = '';
        // $res.='<h3>'.$this->url('store').'</h3>'; //4 debug
        $res .= Form::bsOpenPanel($this->panelContract, 'update');
    ...
    public function getFormData(array $params = []): array
    {
        ...
        $row = $params['row'] ?? $this->panelContract->getRow();
        foreach ($fields as $field) {
    ...
    */
    ...
    public function exceptFields(string $act): DataCollection
    {
        ...
        $fields = collect($panel->fields())
            ->filter(
                function ($item) use ($excepts, $act): bool {

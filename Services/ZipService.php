...
class ZipService
{
    ...
    {
        $zipService = self::getInstance();
        $zipService->filename_zip = $filename_zip;
    ...
    public static function getFilenameZip(): string
    {
        ...
        return $zipService->filename_zip;
    }
    ...
    {
        $zipArchive = self::getZipArchive();
        foreach ($files as $index => $path) {
            $path = str_replace(['/', '\\'], [\DIRECTORY_SEPARATOR, \DIRECTORY_SEPARATOR], (string) $path);
            if (isset($names)) {
                $zipArchive->addFile($path, $names[$index]);
            } else {
    ...
    {
        $filenameZipPath = self::getFilenameZipPath();

        return response()->download($filenameZipPath);
    }
